...
class ParcoursRunner(Node):

    # ── Tuneable parameters ─────────────────────────────────────────── #

    TOTAL_HOOPS = 4

    # HSV range for "Gazebo green"
    ...
    MAX_YAW_SPEED = 0.16
    MAX_ALT_SPEED = 0.16
    FORWARD_FAST = 0.04           # Well-aligned, hoop far away
    FORWARD_SLOW = 0.03          # Roughly aligned
    TRAVERSE_SPEED = 0.15         # Gentle push through hoops
    SEARCH_SPEED = 0.02           # Yaw while searching

    # Alignment tolerance (pixels from frame center)
    ALIGN_TOL_X = 50              # "well-aligned" zone
    ALIGN_TOL_Y = 40
    COARSE_TOL_X = 150            # Beyond this → barely creep forward
    COARSE_TOL_Y = 80             # Tightened: stop forward if vert off
    ...
    def __init__(self):
        super().__init__('parcours_runner')

        # ROS interfaces
        self.camera_sub = self.create_subscription(
            Image, '/drone1/image_raw', self._camera_cb, 10)

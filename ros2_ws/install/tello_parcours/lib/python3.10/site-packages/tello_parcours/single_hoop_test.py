...
class SingleHoopTest(Node):
    ...
    COARSE_TOL_Y = 150            # Relaxed (Was 80)
    ...
    def __init__(self):
        super().__init__('single_hoop_test')
        ...
        # Monitor flight data to ensure we are connected
        self.flight_data_sub = self.create_subscription(
            FlightData, '/flight_data', self._flight_data_cb, 10)
        ...
        self.hoop_area = 0
        self.battery = -1         # Connection indicator
        self.last_flight_data_time = self.get_clock().now()
    ...
    def _flight_data_cb(self, msg: FlightData):
    ...
    def _control_loop(self):
        ...
        if self.state == 'INIT':
            self.ticks += 1
            # Wait for battery data to ensure Tello is linked
            if self.battery > 0:
                if self.ticks >= 20: 
                    self.state = 'TAKEOFF'
                    self.ticks = 0
                ...
                if self.ticks % 20 == 0:
                    self.get_logger().info('Wachten op verbinding met drone (check /flight_data)...')

        # ---- TAKEOFF ------------------------------------------------ #
        elif self.state == 'TAKEOFF':
            self.ticks += 1
            # Retry takeoff every 2 seconds until drone is in the air
            if self.ticks % 20 == 1:
                self.get_logger().info('Opstijgen command versturen...')
                self._tello_action('takeoff')
            
            # If we see height > 0 (or enough time passed), we are airborne
            # Note: real tello takes ~3-4s to takeoff
            if self.ticks >= 60:
                self.get_logger().info('Gestabiliseerd — zoeken naar hoepel 1')
                self.state = 'SEARCH'
                self.ticks = 0
